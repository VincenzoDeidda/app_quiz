...
//funzione di extraTime,invocato da eventoClick
  function extraTime(){
    ...
    <div className="container-gioco">
      ...
      <div className="app">
        {mostraPunteggio ? (
          ...
          <br></br>
          <br></br>
            {winner ? <h1 className="btn-success"> HAI VINTO!!<br></br>Carlo Conti is dead, long live Carlo Conti.</h1> : <h1 className="btn-danger">Hai perso!!! Mi dispiace!</h1>}
        <h3>Il tuo punteggio è {punteggio} di 10</h3> 
          ...
          <>
            <div className="domanda">
              ...
              <svg width="1em" height="1em" viewBox="0 0 20 20" class="bi bi-alarm" fill="red" xmlns="http://www.w3.org/2000/svg">
              ...
              <div>
                <div className="testo-domanda">
                  {domande[domandaCorrente].testoDomanda}
                </div>
              </div>
              <div className="row">
                {/* creazione delle opzioni di risposta */}
                {domande[domandaCorrente].risposte.map((risposte) => (
                    <button id="risposta"className="btn btn-primary btn-lg active col-6 col-md-4" onClick={() => handleAnswerOptionClick(risposte.isCorrect)}>{risposte.testoRisposta}</button>))}
              </div> 
              {/* creazione dell'opzione extraTime e tramite operatore ternario disabilito il bottone dopo il primo click */}
              {extTime?(<button id="bnt-1" className="btn btn-danger btn-lg active"onClick={extraTime}>
                extraTime 10s
              </button>):(<button id="bnt-1" disabled className="btn btn-danger btn-lg active"onClick={extraTime}> extraTime 10s</button>)}
            </div>
          </>
        )}
      </div>
